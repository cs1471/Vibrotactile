function spatialLocalizationExperiment(name,exptdesign)
    ...
    for iBlock=1:exptdesign.numBlocks
        ...
        for iTrial=1:exptdesign.numTrialsPerSession
           ...
           if isequal(stimuli(1:2, iTrial),stimuli(3:4,iTrial)) && mod(iBlock,2) % are stimuli the same?
                    correctResponse=1;
           else
                    correctResponse=2;
           end
        ...
        save(['./SpatialLocData/' exptdesign.number '/' datestr(now, 'yyyymmdd_HHMM') '-' exptdesign.subjectName '_block' num2str(iBlock) '.mat'], 'trialOutput', 'exptdesign');
